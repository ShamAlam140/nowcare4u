...
export default function DiabetesRecords() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/diabetes/records`, {
  ...
  const remove = async (id: string) => {
    if (!token) return;
    try {
      const res = await fetch(`${API_BASE_URL}/diabetes/records/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
    ...
    <div className="max-w-4xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="space-x-2">
          <button onClick={() => router.push('/diabetes')} className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">New</button>
