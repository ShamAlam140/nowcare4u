...
const KickCounter: React.FC = () => {
  ...
  const loadUserData = async () => {
    try {
      if (!token) return;
      const res = await fetch(`${API_BASE_URL}/kick-counter/data`, {
        headers: {
  ...
  const saveNameOnly = async () => {
    try {
      ...
      if (!newName) return;
      await fetch(`${API_BASE_URL}/kick-counter/data`, {
        method: 'POST',
        headers: {
  ...
  const loadNotificationSettings = async () => {
    try {
      if (!token) return;
      const res = await fetch(`${API_BASE_URL}/kick-counter/notifications`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const setupNotifications = async () => {
    try {
      ...
      await fetch(`${API_BASE_URL}/kick-counter/notifications`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const sharePdf = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/kick-counter/share-pdf`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      <Card className="bg-white/80 backdrop-blur border border-gray-100 shadow-lg rounded-2xl">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => router.push('/kick-records')}>
              Records
            </Button>
