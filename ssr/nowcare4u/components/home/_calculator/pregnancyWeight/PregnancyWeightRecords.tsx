...
export default function PregnancyWeightRecords() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/pregnancy-weight/record`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const addEntry = async () => {
    if (!token) return;
    try {
      const res = await fetch(`${API_BASE_URL}/pregnancy-weight/entry`, {
        method: 'POST',
  ...
  const resetAll = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/pregnancy-weight/record`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
    ...
    <div className="max-w-4xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="space-x-2">
          <button onClick={() => router.push('/pregnancy-weight')} className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Update Baseline</button>
      ...
      {!loading && !baseline && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-xl">
          ...
          <div>
            <button onClick={() => router.push('/pregnancy-weight')} className="mt-3 px-3 py-2 rounded bg-blue-600 text-white">Set Baseline</button>
          </div>
        </div>
      )}
