...
export default function PregnancyWeightCalc() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!token) return;
      try {
        const res = await fetch(`${API_BASE_URL}/pregnancy-weight/record`, {
  ...
  const initialize = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/pregnancy-weight/baseline`, {
        method: 'POST',
        headers: {
      ...
      router.push('/pregnancy-weight-records');
    } catch (e: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => router.push('/pregnancy-weight-records')} className="px-3 py-2 text-sm rounded bg-gray-100 hover:bg-gray-200">Records</button>
      </div>
